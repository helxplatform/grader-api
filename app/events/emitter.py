...
from app.schemas import PydanticEvent
...
class PydanticEventEmitter(EventEmitter):
    def _validate_pydantic_event(self, event: PydanticEvent) -> tuple[str, dict]:
        ...
        name, payload = event.event_name, event
        return name, payload
    
    def on(self, event: str | PydanticEvent, *args, **kwargs) -> Callable:
        # Support passing in an event model as an event name.
        if isinstance(event, PydanticEvent):
    ...
    you must keep in mind that since it's a future, the event is not necessary processed after calling. """
    def emit_future(self, event: PydanticEvent) -> asyncio.Task:
    ...
    async def emit_async(self, event: PydanticEvent) -> None:
        name, payload = self._validate_pydantic_event(event)
